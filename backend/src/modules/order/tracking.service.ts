...
import type { Shipment, Order, ShipmentStatus as PrismaShipmentStatus, Prisma } from '@prisma/client';
...
import {
  ...
  STALE_SHIPMENT_HOURS,
  MAX_SYNC_FAILURES_BEFORE_FLAG,
...
const CARRIER_META: Record<CarrierCode, { name: string; trackingUrl: (awb: string) => string }> = {
...
export const SUPPORTED_CARRIERS = CARRIER_CODES.map((code) => ({ code, ...CARRIER_META[code] }));
 ...
 */
export function isPlausibleAwbFormat(awb: string): boolean {
  if (!awb) return false;
  if (awb.length < 6 || awb.length > 40) return false;
  return /^[A-Z0-9-]+$/.test(awb);
}

// ---------------------------------------------------------------------------
// Unified tracking status — normalizes across carriers (unchanged)
// ---------------------------------------------------------------------------
...
export type TrackingStatus =
  ...
  | 'DeliveryFailed' // A delivery attempt was made and failed (recipient unavailable, refused, address issue) — usually retried
  | 'Exception' // Something else went wrong (customs hold, damage, lost) — not simply a failed attempt
  | 'Returned'; // Returned to sender

/** Maps our normalized tracking status to the granular, courier-derived ShipmentStatus. */
const TRACKING_STATUS_TO_SHIPMENT_STATUS: Record<TrackingStatus, PrismaShipmentStatus> = {
 ...
 */
function normalizeTrack17Status(stage: string | undefined, subStatus: string | undefined): TrackingStatus | null {
  if (subStatus === 'InTransit_PickedUp') return 'PickedUp';
  if (subStatus === 'Exception_Returning' || subStatus === 'Exception_Returned') return 'Returned';
...
// ---------------------------------------------------------------------------
// Simulation engine — generates realistic tracking events (unchanged)
// Kept available for development/testing only, gated by TRACKING_SIMULATE=true
// (see .env.example) — never used when a real provider key is configured.
// ---------------------------------------------------------------------------
...
interface SimEvent {
  ...
  description: string;
  location: string;
  delayMinutes: number; // delay from AWB verification
}
...
function generateSimulationEvents(sinceTime: Date): Array<{ status: TrackingStatus; description: string; location: string; eventTime: Date }> {
 ...
 */
const TRACK17_CARRIER_CODES: Partial<Record<CarrierCode, number>> = {
  DELHIVERY: 100060,
  BLUEDART: 100055,
  DTDC: 100069,
  INDIA_POST: 9021,
...
function getTrack17CarrierCode(carrierCode: string): number | null {
  return TRACK17_CARRIER_CODES[carrierCode as CarrierCode] ?? null;
}

/** Reverse lookup — a 17TRACK numeric carrier code back to one of our CarrierCodes, for logging/mismatch messages. */
function findCarrierCodeByTrack17Code(track17Code: number): CarrierCode | null {
  const entry = Object.entries(TRACK17_CARRIER_CODES).find(([, v]) => v === track17Code);
  return (entry?.[0] as CarrierCode) || null;
}

interface Track17Response<T = unknown> {
  code: number;
  data?: T;
}

interface Track17RegisterResultItem {
  number: string;
  carrier: number;
  origin?: number;
}

interface Track17RejectedItem {
  number: string;
  carrier: number;
...
interface Track17Event {
  // ⚠️ Field names below are 17TRACK's documented v2.4 shape — verify
  // against a real payload (see the endpoint comment above) before relying
  ...
  location?: string;
  address?: string;
  stage?: string; // Main status string, e.g. "InTransit"
  sub_status?: string; // e.g. "InTransit_PickedUp"
}
...
async function track17Request<T = unknown>(
  path: string,
  body: unknown
): Promise<{ ok: true; body: Track17Response<T> } | { ok: false; status: number; message: string }> {
  const token = env.tracking.apiKey;
  if (!token) {
  ...
  try {
    ...
    const parsed = (await res.json().catch(() => null)) as Track17Response<T> | null;
    if (!parsed) return { ok: false, status: res.status, message: `Non-JSON response (HTTP ${res.status})` };
    ...
    if (parsed.code === 0) return { ok: true, body: parsed };
...
function mapTrack17ItemToEvents(item: Track17AcceptedItem): NormalizedEvent[] {
 ...
 */
async function track17Register(
  carrier17Code: number | undefined,
  trackingNumber: string
): Promise<{ outcome: 'found'; carrier: number } | { outcome: 'not_found' } | { outcome: 'error'; message: string }> {
  const body = [{ number: trackingNumber, ...(carrier17Code ? { carrier: carrier17Code } : {}) }];
  const result = await track17Request<{ accepted: Track17RegisterResultItem[]; rejected: Track17RejectedItem[] }>('/register', body);
  if (!result.ok) {
    logger.warn(`17TRACK register failed for ${trackingNumber}: ${result.message}`);
    return { outcome: 'error', message: result.message };
  }

  const accepted = result.body.data?.accepted?.[0];
  if (accepted) return { outcome: 'found', carrier: accepted.carrier };
...
async function track17GetTrackInfo(carrier17Code: number, trackingNumber: string): Promise<Track17AcceptedItem | null> {
  const result = await track17Request<{ accepted: Track17AcceptedItem[]; rejected: Track17RejectedItem[] }>('/gettrackinfo', [
    { number: trackingNumber, carrier: carrier17Code },
  ]);
  if (!result.ok) {
    logger.warn(`17TRACK gettrackinfo failed for ${trackingNumber}: ${result.message}`);
    return null;
  }
...
// ---------------------------------------------------------------------------
// AWB verification (requirement #3) — never trust seller input alone.
// ---------------------------------------------------------------------------

export type VerifyAwbResult =
  | { outcome: 'verified'; verifiedCarrierCode: string }
  | { outcome: 'not_found' }
  | { outcome: 'mismatch'; detectedCarrierNames: string[] }
  | { outcome: 'deferred'; reason: string };

export async function verifyAwbWithProvider(carrierCode: string, awb: string, _carrierName?: string): Promise<VerifyAwbResult> {
  ...
  if (result.outcome === 'error') return { outcome: 'deferred', reason: result.message };
...
// ---------------------------------------------------------------------------
// Core sync engine — shared by the cron job and the 17TRACK webhook
// (unchanged from the KeyDelivery version — provider-agnostic)
// ---------------------------------------------------------------------------

type OrderWithItems = Order & { items: Array<{ product: { sellerId: string } }> };

function mergeRiskFlags(existing: string[], additions: string[]): string[] {
  return Array.from(new Set([...existing, ...additions]));
}

const TIME_BASED_RISK_FLAGS: string[] = [RISK_FLAGS.PICKUP_OVERDUE, RISK_FLAGS.STALE_NO_MOVEMENT];
 ...
 */
function checkTimeBasedRisk(shipment: Pick<Shipment, 'status' | 'createdAt' | 'pickupConfirmedAt' | 'verified'>): string[] {
  const flags: string[] = [];
  const now = Date.now();

  if (TERMINAL_SHIPMENT_STATUSES.includes(shipment.status)) return flags;

  if (!shipment.pickupConfirmedAt && shipment.verified) {
    const hoursSinceCreated = (now - shipment.createdAt.getTime()) / (1000 * 60 * 60);
    if (hoursSinceCreated > PICKUP_GRACE_HOURS) flags.push(RISK_FLAGS.PICKUP_OVERDUE);
  }
  ...
  if (hoursSinceMovement > STALE_SHIPMENT_HOURS) flags.push(RISK_FLAGS.STALE_NO_MOVEMENT);
...
async function applyProviderEvents(params: {
  ...
  source: 'TRACKING_API' | 'SIMULATION';
}): Promise<void> {
  ...
  const stickyFlags: Set<string> = new Set(shipment.riskFlags.filter((f: string) => !TIME_BASED_RISK_FLAGS.includes(f)));
  let nextStatus: PrismaShipmentStatus = shipment.status;
  ...
  let priorMeaningfulEventCount = await prisma.shipmentEvent.count({
    where: { shipmentId: shipment.id, status: { not: 'InfoReceived' } },
  });

  const auditEntries: Array<{ action: string; previousState?: string; newState?: string; metadata?: Record<string, unknown> }> = [];

  if (!TERMINAL_SHIPMENT_STATUSES.includes(shipment.status)) {
    for (const evt of newEvents) {
      const mapped = TRACKING_STATUS_TO_SHIPMENT_STATUS[evt.status];
      const currentRank = SHIPMENT_STATUS_RANK[nextStatus];
      const mappedRank = SHIPMENT_STATUS_RANK[mapped];
      if (mappedRank < currentRank) continue; // never downgrade

      if (mapped === 'PICKUP_CONFIRMED' && !pickupConfirmedAt) {
        pickupConfirmedAt = evt.eventTime;
        auditEntries.push({ action: AUDIT_ACTIONS.PICKUP_CONFIRMED, previousState: nextStatus, newState: mapped, metadata: { eventTime: evt.eventTime } });
      }
      ...
      if (mapped === 'DELIVERED' && !deliveredAt) {
        ...
        auditEntries.push({ action: AUDIT_ACTIONS.OUT_FOR_DELIVERY, previousState: nextStatus, newState: mapped });
      } else if (mapped === 'DELIVERY_FAILED') {
        auditEntries.push({ action: AUDIT_ACTIONS.DELIVERY_FAILED, previousState: nextStatus, newState: mapped });
      } else if (mapped === 'RETURNED') {
        auditEntries.push({ action: AUDIT_ACTIONS.RETURNED, previousState: nextStatus, newState: mapped });
      } else if (mapped !== nextStatus) {
        auditEntries.push({ action: AUDIT_ACTIONS.TRACKING_UPDATED, previousState: nextStatus, newState: mapped });
      }

      nextStatus = mapped;
      if (evt.status !== 'InfoReceived') priorMeaningfulEventCount += 1;
    }
  }
  ...
  const updatedShipmentData: Prisma.ShipmentUpdateInput = {
    status: nextStatus,
    pickupConfirmedAt,
    deliveredAt,
    deliverySource,
    deliveryEvidence: (deliveryEvidence ?? undefined) as Prisma.InputJsonValue | undefined,
  ...
  const savedShipment =
    newEvents.length > 0
      ...
      : await prisma.shipment.update({ where: { id: shipment.id }, data: updatedShipmentData });
  ...
  const targetOrderStatus = SHIPMENT_STATUS_TO_ORDER_STATUS[nextStatus];
  ...
  if (!orderStatusChanged) {
    await prisma.order.update({ where: { id: order.id }, data: { lastTrackingSync: new Date() } });
  }
  ...
  if (newEvents.length > 0) {
    ...
    emitOrderUpdate({ ...order, status: orderStatusChanged ? (targetOrderStatus ?? order.status) : order.status });
    notifyUser({
      userId: order.userId,
      type: 'ORDER_STATUS',
...
export async function syncTracking(orderId: string): Promise<void> {
  ...
  if (!shipment.verified) {
    if (shipment.verificationAttempts >= MAX_VERIFICATION_ATTEMPTS) return;
    ...
    if (result.outcome === 'verified') {
      ...
      await recordAudit({ orderId, shipmentId: shipment.id, action: AUDIT_ACTIONS.AWB_VERIFIED, source: 'SYSTEM', newState: 'AWB_VERIFIED' });
        ...
        data: {
          ...
          riskFlags:
            result.outcome === 'not_found'
              ? mergeRiskFlags(shipment.riskFlags, [RISK_FLAGS.AWB_NOT_FOUND])
              : result.outcome === 'mismatch'
                ? mergeRiskFlags(shipment.riskFlags, [RISK_FLAGS.CARRIER_MISMATCH])
                : mergeRiskFlags(shipment.riskFlags, [RISK_FLAGS.PROVIDER_UNAVAILABLE]),
        },
      });
  ...
  let rawEvents: NormalizedEvent[];
  const source: 'TRACKING_API' | 'SIMULATION' = env.tracking.simulate ? 'SIMULATION' : 'TRACKING_API';
    ...
    if (!item) {
      ...
      await prisma.shipment.update({
        ...
        data: {
          ...
          lastSyncedAt: new Date(),
          ...(failures >= MAX_SYNC_FAILURES_BEFORE_FLAG
            ? { flaggedForReview: true, riskFlags: mergeRiskFlags(shipment.riskFlags, [RISK_FLAGS.TRACKING_API_INCONSISTENT]) }
            : {}),
        },
      });
      if (failures >= MAX_SYNC_FAILURES_BEFORE_FLAG) {
        await recordAudit({ orderId, shipmentId: shipment.id, action: AUDIT_ACTIONS.SYNC_FAILED, source: 'SYSTEM', metadata: { failures } });
      }
      return;
    }
  ...
  if (rawEvents.length === 0) {
    ...
    const stickyFlags: string[] = shipment.riskFlags.filter((f: string) => !TIME_BASED_RISK_FLAGS.includes(f));
    await prisma.shipment.update({
      where: { id: shipment.id },
  ...
  await applyProviderEvents({ order: order as OrderWithItems, shipment, existingEventKeys, rawEvents, source });
}
 ...
 * comparison.
 */
export function verifyTrack17WebhookToken(token: string | undefined | null): boolean {
  const expected = env.tracking.webhookToken;
 ...
 * ignored.
 */
export async function handleTrack17WebhookPayload(body: unknown): Promise<{ handled: boolean; reason?: string }> {
  const payload = body as { event?: string; data?: { accepted?: Track17AcceptedItem[] } };
  ...
  for (const item of payload.data.accepted) {
    ...
    await (async () => {
      ...
      const shipmentWithOrder = await prisma.shipment.findFirst({
        where: { normalizedAwb },
        orderBy: { createdAt: 'desc' },
        include: { events: true, order: { include: { items: { include: { product: { select: { sellerId: true } } } } } } },
      });
      if (!shipmentWithOrder) {
        logger.warn(`17TRACK webhook: no shipment found for AWB ${item.number}`);
        return;
      }
      if (TERMINAL_SHIPMENT_STATUSES.includes(shipmentWithOrder.status) || shipmentWithOrder.order.status === 'CANCELLED') {
        return;
      }
      ...
      if (!verified) {
        ...
        status = 'AWB_VERIFIED';
        await prisma.shipment.update({ where: { id: shipmentWithOrder.id }, data: { verified, verifiedCarrierCode, status } });
      }
      ...
      await applyProviderEvents({
        ...
        shipment: { ...shipmentFields, verified, verifiedCarrierCode, status } as Shipment,
        existingEventKeys,
        rawEvents,
        source: 'TRACKING_API',
      });
